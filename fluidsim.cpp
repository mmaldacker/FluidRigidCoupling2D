...
void FluidSim::initialize(float width, int ni_, int nj_) {
  ...
  dv.resize(ni, nj + 1);
  solid_u.resize(ni + 1, nj);
  solid_v.resize(ni, nj + 1);
  solid_u.set_zero();
  solid_v.set_zero();
  rigid_u_weights.resize(ni + 1, nj);
  rigid_v_weights.resize(ni, nj + 1);
  ...
  for (int j = 0; j < u.nj; ++j)
    for (int i = 0; i < u.ni; ++i) {
      if (u_weights(i, j) == 0) {
        ...
        double solid_phi_val =
            interpolate_gradient(solid_normal, pos / dx, nodal_solid_phi);
        normalize(solid_normal);

        Vec2f rigid_normal(0, 0);
        double rigid_phi_val =
            interpolate_gradient(rigid_normal, pos / dx, nodal_rigid_phi);
...
void FluidSim::process_collisions() {
  ...
  // Should really probably find first collision point and back up to that
  // point, rather than trying to project out of walls.
  ...
  while (penetrating_points.size() > 0) {
    ...
    for (unsigned int i = 0; i < penetrating_points.size(); ++i) {
      ...
      phi = interpolate_normal(normal, vertex, nodal_solid_phi,
                               0.5f * dx * Vec2f(1, 1), dx);
  ...
  do {
    ...
    for (unsigned int i = 0; i < vertices.size(); ++i) {
      ...
      Vec2f vertex(vertices[i][0], vertices[i][1]);
      phi =
          interpolate_normal(normal, vertex, nodal_solid_phi, Vec2f(0, 0), dx);
...
void FluidSim::update_rigid_body_grids() {
  ...
  // recompute the grid-based "effective" masses per axis, so that we can
  // *exactly* balance in hydrostatic scenarios.
  double u_sum =
      std::accumulate(rigid_u_weights.a.begin(), rigid_u_weights.a.end(), 0.0);
  double v_sum =
      std::accumulate(rigid_v_weights.a.begin(), rigid_v_weights.a.end(), 0.0);
...
void FluidSim::recompute_solid_velocity() {
  ...
  for (int i = 0; i < ni + 1; ++i) {
    for (int j = 0; j < nj - 1; ++j) {
      Vec2f pos(i * dx, (j + 0.5f) * dx);
      if (0.5f * (nodal_solid_phi(i, j) + nodal_solid_phi(i, j + 1)) <
          0.5f * (nodal_rigid_phi(i, j) + nodal_rigid_phi(i, j + 1)))
        solid_u(i, j) = 0;
      else {
  ...
  for (int i = 0; i < ni - 1; ++i) {
    for (int j = 0; j < nj + 1; ++j) {
      Vec2f pos((i + 0.5f) * dx, j * dx);
      if (0.5f * (nodal_solid_phi(i, j) + nodal_solid_phi(i + 1, j)) <
          0.5f * (nodal_rigid_phi(i, j) + nodal_rigid_phi(i + 1, j)))
        solid_v(i, j) = 0;
      else {
...
void FluidSim::accumulate(Array2f &accum, float q, int i, int j, float fx,
                          float fy) {
  float weight;
...
void FluidSim::update_from_grid(float alpha) {
  for (int p = 0; p < particles.size(); ++p) {
    ...
    particles_velocity[p][0] =
        alpha * pic_vel[0] + (1.0f - alpha) * flip_vel[0];
    particles_velocity[p][1] =
        alpha * pic_vel[1] + (1.0f - alpha) * flip_vel[1];
  }
}
  ...
  for (int j = 0; j < nj; ++j) {
    for (int i = 0; i < ni; ++i) {
      if (liquid_phi(i, j) < 0.5 * dx) {
        float solid_phi_val =
            0.25f * (nodal_solid_phi(i, j) + nodal_solid_phi(i + 1, j) +
...
void FluidSim::apply_projection(float dt) {
  ...
  solve_pressure(dt); // original Batty '07 style: SPD, but with dense blocks
                      // due to J'MJ terms
}
...
void FluidSim::compute_pressure_weights() {
  ...
  for (int j = 0; j < u_weights.nj; ++j)
    for (int i = 0; i < u_weights.ni; ++i) {
      u_weights(i, j) =
          1 -
          fraction_inside(nodal_solid_phi(i, j + 1), nodal_solid_phi(i, j)) -
          rigid_u_weights(i, j);
  ...
  for (int j = 0; j < v_weights.nj; ++j)
    for (int i = 0; i < v_weights.ni; ++i) {
      v_weights(i, j) =
          1 -
          fraction_inside(nodal_solid_phi(i + 1, j), nodal_solid_phi(i, j)) -
          rigid_v_weights(i, j);
      v_weights(i, j) = clamp(v_weights(i, j), 0.0f, 1.0f);
    }
}

void FluidSim::compute_base_trans(Array2d &base_trans_x, Array2d &base_trans_y,
                                  Array2d &base_rot_z) {
  assert(rbd);
  Vec2f centre_of_mass;
  rbd->getCOM(centre_of_mass);

  base_trans_x.set_zero();
  base_trans_y.set_zero();
  base_rot_z.set_zero();
  ...
  for (int j = 0; j != nj; ++j) {
    for (int i = 0; i != ni; ++i) {
      ...
      if (centre_phi < 0) {
        double u_term = (rigid_u_weights(i + 1, j) - rigid_u_weights(i, j));
        double v_term = (rigid_v_weights(i, j + 1) - rigid_v_weights(i, j));

        // Translation coupling
        base_trans_x(i, j) = u_term / dx;
        base_trans_y(i, j) = v_term / dx;

        // Rotation coupling
        Vec2f position((i + 0.5f), (j + 0.5f));
        Vec2f rad = position - centre_of_mass / dx;
...
void FluidSim::solve_pressure(float dt) {
  ...
  if (rbd) {
    compute_base_trans(base_trans_x, base_trans_y, base_rot_z);
  }
          ...
          for (int k = 0; k < ni; ++k) {
            for (int m = 0; m < nj; ++m) {
              ...
              if (other_phi < 0) {
                // Translation
                val +=
                    dt * base_trans_x(i, j) * base_trans_x(k, m) / rigid_u_mass;
                val +=
                    dt * base_trans_y(i, j) * base_trans_y(k, m) / rigid_v_mass;
  ...
  for (int j = 0; j < u.nj; ++j)
    for (int i = 1; i < u.ni - 1; ++i) {
      int index = i + j * ni;
      if (u_weights(i, j) > 0 &&
          (liquid_phi(i, j) < 0 || liquid_phi(i - 1, j) < 0)) {
        float theta = 1;
        ...
        if (theta < 0.01f)
          theta = 0.01f;
        u(i, j) -=
            dt * (float)(pressure[index] - pressure[index - 1]) / dx / theta;
        u_valid(i, j) = 1;
      } else
        u(i, j) = 0;
    }
  ...
  for (int j = 1; j < v.nj - 1; ++j)
    for (int i = 0; i < v.ni; ++i) {
      int index = i + j * ni;
      if (v_weights(i, j) > 0 &&
          (liquid_phi(i, j) < 0 || liquid_phi(i, j - 1) < 0)) {
        float theta = 1;
        ...
        if (theta < 0.01f)
          theta = 0.01f;
        v(i, j) -=
            dt * (float)(pressure[index] - pressure[index - ni]) / dx / theta;
        v_valid(i, j) = 1;
      } else
        v(i, j) = 0;
    }
  ...
  if (rbd) {
    ...
    for (int j = 0; j < nj; ++j) {
      for (int i = 0; i < ni; ++i) {
        ...
        float centre_phi = liquid_phi(i, j);
        if (centre_phi < 0) {
          updated_rigid_linear_velocity[0] +=
              (float)(dt * base_trans_x(i, j) * pressure[index] / rigid_u_mass);
          updated_rigid_linear_velocity[1] +=
              (float)(dt * base_trans_y(i, j) * pressure[index] / rigid_v_mass);

          updated_rigid_angular_momentum +=
              (float)(dt * base_rot_z(i, j) * pressure[index]);
        }
      }
    }
